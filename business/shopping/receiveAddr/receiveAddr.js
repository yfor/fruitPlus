	...
	var ctrl = ["$scope","$state","$stateParams","$http","$q",function($scope,$state, $stateParams,$http,$q){
		...
		var provinceName = ["北京市","上海市","天津市","重庆市","河北省","山西省","辽宁省","吉林省","河南省","江苏省","浙江省","安徽省","福建省","江西省","山东省","湖北省","湖南省","广东省","海南省","四川省","贵州省","云南省","陕西省","甘肃省","青海省","黑龙江省","内蒙古自治区","广西壮族自治区","西藏自治区","宁夏回族自治区","新疆维吾尔自治区","台湾省","香港特别行政区","澳门特别行政区","其它"]; 
		 ...
		 $("#province").change(function(){
		 	cityName($(this).context.value)
		 	$scope.currentuser.province = $(this).context.value;
		 })
		 $("#city").change(function(){
		 	$scope.currentuser.city = $(this).context.value;
		 })
		...
		$scope.saveUserAddr = function(userId){
			...
			if($scope.currentuser.name==""||$scope.currentuser.phone==""||$scope.currentuser.details==""){
				...
				var data = {
				    "address": {
				        "mobile_number": $scope.currentuser.phone,
				        "detailed_address": $scope.currentuser.province+$scope.currentuser.city+$scope.currentuser.details+ "收货人 ："+$scope.currentuser.name +"收货人电话 " +$scope.currentuser.phone,
				        "is_default": true
				    },
